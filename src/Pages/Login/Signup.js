...
const Signup = () => {
    ...
    return (
        <section className='bg w-full lg:max-w-screen-2xl mx-auto my-5 p-10'>
            <h2 className='text-center w-1/5 mx-auto mb-5 font-bold text-3xl border border-gray-500 rounded-lg py-4'>SIGNUP</h2>
            <form onSubmit={handleSignup}>
                <label className="block">
                    <span className="after:content-['*'] after:ml-0.5 after:text-red-500 block text-sm font-medium text-slate-700">
                        Name
                    </span>
                    <input type="text" name="name" className="mt-1 px-3 py-2 bg-white border shadow-xl border-black placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="John Doe" />
                </label>
                ...
                <label className="block">
                    <input type="submit" className="px-3 py-2 shadow-xl block w-2/5 bg-gray-900 text-white font-semibold text-xl hover:bg-gray-500 hover:text-black ease-in duration-300 border border-slate-500 mx-auto rounded-md" value="Signup" />
                </label>
            </form>
            ...
            <div className='text-center mt-5'>
                <p>Already have an account? <Link to='/login' className='font-semibold underline underline-offset-4 text-blue-700'>Login</Link></p>
            </div>
        </section>
    );
};
